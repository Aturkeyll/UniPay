...
// --- Security tables (migration 005) --------------------------------------
echo "AUTHENTICATION AND QUOTES\n";

if (!in_array('student_login_tokens', $tables, true)) {
    apply($pdo, 'create student_login_tokens', "
    ...
    echo "  [  OK   ] student_login_tokens exists\n"; $already++;
}

if (!in_array('quotes', $tables, true)) {
    apply($pdo, 'create quotes', "
        CREATE TABLE quotes (
            id INT AUTO_INCREMENT PRIMARY KEY,
    ...
    echo "  [  OK   ] quotes exists\n"; $already++;
}

// Staff login failures are logged here with a NULL student_number.
$lacols = columns($pdo, 'lookup_attempts');
if (($lacols['student_number'] ?? '') !== 'varchar(20)') {
    apply($pdo, 'widen lookup_attempts.student_number',
        "ALTER TABLE lookup_attempts MODIFY COLUMN student_number VARCHAR(20) NULL");
} else {
    echo "  [  OK   ] lookup_attempts.student_number is VARCHAR(20)\n"; $already++;
}
echo "\n";
